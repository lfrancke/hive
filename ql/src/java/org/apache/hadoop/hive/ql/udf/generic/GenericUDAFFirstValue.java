...
public class GenericUDAFFirstValue extends AbstractGenericUDAFResolver {
  ...
  static class FirstValStreamingFixedWindow extends GenericUDAFStreamingEvaluator<Object> {
    ...
    class State extends GenericUDAFStreamingEvaluator<Object>.StreamingState {
      ...
      State(int numPreceding, int numFollowing, AggregationBuffer buf) {
    ...
    FirstValStreamingFixedWindow(GenericUDAFEvaluator wrappedEval, int numPreceding,
      int numFollowing) {
    ...
    public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException {
      ...
      if (s.numRows >= s.numFollowing) {
         ...
         */
        if (fb.skipNulls && s.valueChain.isEmpty()) {
          s.results.add(null);
        } else {
      ...
      if (!s.valueChain.isEmpty()) {
        int fIdx = s.valueChain.getFirst().idx;
    ...
    public Object terminate(AggregationBuffer agg) throws HiveException {
      State s = (State) agg;
      FirstValueBuffer fb = (FirstValueBuffer) s.wrappedBuf;
      ValIndexPair r = fb.skipNulls && s.valueChain.isEmpty() ? null : s.valueChain.getFirst();
      ...
      for (int i = 0; i < s.numFollowing; i++) {
        ...
        if (r != null) {
          ...
          if (s.numPreceding != BoundarySpec.UNBOUNDED_AMOUNT
            ...
            s.valueChain.removeFirst();
            r = s.valueChain.isEmpty() ? r : s.valueChain.getFirst();
          }
        }
      }
